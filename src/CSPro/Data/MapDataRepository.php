...
class MapDataRepository {
        ...
        try {
            ...
            if (!$fromAreaNames) {
                ...
                if ($isConnected === true) {
                    ...
                    $e =  new \Exception('Failed connecting to database.');
    ...
    public function getCaseUuids($dictionaryName, $ids) {
        $e = null;
        $caseGuids = [];
        $curLevel = isset($ids)? count($ids) : 0;
        $dictionaryHelper = $this->getDictionaryHelper();
...
//get the first id item name convert to lower and set the where clause
        ...
        if ($latList !== false && $longList !== false) {
            ...
            if (isset($latList[0]) && !isset($latList[1])) {//id item
                ...
                $stm = "SELECT `level-1`.`case-id` as `uuid` " . $strLatLongQuery;
                ...
                $stm = "SELECT `level-1`.`case-id` as `uuid`, " . $strLatLongQuery . " FROM `$recordName` JOIN `level-1`  ON  `level-1`.`level-1-id` = $recordName.`level-1-id`  ";
                if($strWhereIds != "") {

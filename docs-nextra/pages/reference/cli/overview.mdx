...
Guide des commandes en ligne de commande (CLI) de CSWeb Community Platform.

## Introduction

CSWeb Community Platform fournit des **commandes CLI Symfony** pour gérer le breakout, vérifier la configuration, et administrer la plateforme.
...
Arguments:
  dictionnaires         Comma-separated list of dictionaries to process
...
Toutes les commandes supportent les niveaux de verbosité :
...
Les commandes CLI utilisent les variables du fichier `.env` :
...
Les commandes écrivent dans les logs Symfony :
...
```bash
#!/bin/bash
# breakout_all.sh - Breakout tous les dictionnaires

DICTS=("EVAL_DICT" "KAIROS_DICT" "CENSUS_DICT")
...
echo "All dictionaries processed!"
```

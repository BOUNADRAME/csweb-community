...
Commande CLI pour vérifier les drivers de base de données disponibles.

## Description

La commande `csweb:check-database-drivers` vérifie quels drivers PDO sont installés et disponibles pour CSWeb Community Platform.
...
Tester les connexions aux bases de données configurées.
...
⚠️  WARNING: pdo_pgsql driver is not installed.
   PostgreSQL breakout will not work.

   To install:
   docker-compose exec csweb apt-get install php8.1-pgsql
...
✅ MySQL Metadata    : Connected to mysql:3306/csweb_metadata
❌ PostgreSQL Breakout : Connection FAILED
...
Après installation de CSWeb Community Platform :
...
### 2. Debug Problème de Connexion

Si breakout échoue avec erreur driver :
...
### 3. Changement de SGBD

Après changement de `BREAKOUT_DB_TYPE` dans `.env` :
...
### 4. Installation Driver Manquant

Si driver manquant détecté :
...
La commande lit les variables suivantes depuis `.env` :

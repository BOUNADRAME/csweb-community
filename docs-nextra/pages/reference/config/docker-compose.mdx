...
Guide de configuration du fichier `docker-compose.yml`.

## Vue d'Ensemble

Le fichier `docker-compose.yml` définit tous les services de CSWeb Community Platform :
...
services:
  ...
  csweb:
    ...
    volumes:
      ...
      - csweb_logs:/var/www/html/var/logs
    environment:
  ...
  mysql-breakout:
    image: mysql:8.0
    container_name: csweb-mysql-breakout
    ...
    volumes:
      - mysql_breakout_data:/var/lib/mysql
    networks:
      - csweb-network
  ...
  sqlserver:
    ...
    container_name: csweb-sqlserver
  ...
  phpmyadmin:
    ...
    container_name: csweb-phpmyadmin
    profiles: ["dev"]
    ports:
...
csweb:
  ...
  container_name: csweb
  ports:
    - "${CSWEB_PORT:-8080}:80"  # Port configurable
  volumes:
    - ./:/var/www/html        # Code source (hot reload)
    - csweb_files:/var/www/html/files  # Fichiers persistants
  environment:
    - APP_ENV=${APP_ENV}      # Variables d'environnement
  depends_on:
    - mysql                   # Attend MySQL au démarrage
  networks:
    - csweb-network
```
...
Les **profils** permettent d'activer seulement les services nécessaires.
...
Profil `dev` active les outils de développement :
...
volumes:
  ...
  mysql_data:         # Données MySQL metadata
  postgres_data:      # Données PostgreSQL breakout
...
Toutes les variables du fichier `.env` sont accessibles :
